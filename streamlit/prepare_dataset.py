...
def open_zarr(path: str,
    ...
    """
    Opens a Zarr store from either local disk or S3.
    If `path` is S3 and `aws_creds` is provided, uses them to construct the S3FileSystem.
    """
...
def open_dataarray(path: str, aws_creds: dict | None = None) -> xr.DataArray:
    """
    Opens a stored xarray DataArray (e.g. seasonal_ndvi.nc or monthly_*.nc)
    from local disk or S3.
    """
    if path.startswith("s3://"):
...
def compute_global_band_ranges(
    ...
    """
    For each band found in band_stats.json (located at
    s3://<bucket>/<folder>/<gh>/<gh>_climatology/band_stats.json),
    read that JSON and compute:
       global_min[band] = min( stats[band]['min'] for each gh )
       global_max[band] = max( stats[band]['max'] for each gh )
    Returns two dicts: (global_min, global_max).
    """
    if aws_creds:
...
class ZarrChangePairDataset(Dataset):
    def __init__(
        ...
        """
        # self._orig_zarr_path = zarr_path
    ...
    def _ensure_open(self):
        """
        Called inside the worker's process exactly once, the first time __getitem__
        (or __len__) is invoked. This method:
          1) Builds an S3FileSystem if the path is on S3
          2) Opens the Zarr store via xarray (xr.open_zarr)
          3) Assigns the result to self._ds so subsequent indexing works.
        """
        if self._ds is None:
