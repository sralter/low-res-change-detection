...
def inspect_dataset_for_streamlit(
    geohash: str,
    date0_str: str,
    date1_str: str,
    ...
    """
    1) Open the Zarr on S3 for the given geohash,
    2) Snap date0 and date1 to the nearest time‐indices in the dataset,
    3) Compute:
         • dims, data_vars
         • time_summary (min/max/count)
         • band_summary (per‐band min/max/mean/std/median)
         • three small PNGs under ./inspect_outputs/<geohash>_<date0>_<date1>/:
             – rgb_t0.png   (true‐color at date0)
             – rgb_t1.png   (true‐color at date1)
             – ndvi_diff.png (NDVI(t₁) – NDVI(t₀))
    4) Return all of that in a dict for Streamlit to consume.
    """
    total_steps = 9
    step = 0

    # 1) Load AWS credentials (so we can open Zarr)
    creds = None
    if aws_creds_path and os.path.exists(aws_creds_path):
        creds = json.load(open(aws_creds_path, "r"))
    step += 1
    if progress_callback:
        progress_callback(step / total_steps)

    # # 2) Construct the S3 Zarr path / open it
    # #    (this reuses your open_zarr_from_s3 helper)
    # ds: xr.Dataset = open_zarr_from_s3(
    #     bucket=bucket,
    #     folder=folder,
    #     geohash=geohash,
    #     creds=creds,
    #     consolidated=True
    # )
    # 2) Construct the S3 Zarr‐streamlit path / open it directly
    #    (we bypass open_zarr_from_s3 and just build the mapper ourselves, because
    #     the directory is "<geohash>_zarr_streamlit" now)
    s3_path = f"{bucket}/{folder}/{geohash}/{geohash}_zarr_streamlit"
    opts = {}
    if creds:
        opts = {"key": creds["access_key"], "secret": creds["secret_access_key"]}
    fs = s3fs.S3FileSystem(anon=not bool(creds), **opts)
    ...
    # 3) Identify which dims are time/lat/lon
    dims = dict(ds.dims)
    ...
    # 4) Turn date0_str, date1_str into integer indices
    #    If a user‐passed date is outside the range, we will clamp it.
    def _to_idx(date_str: str) -> int:
        all_times = ds[time_dim].values.astype("datetime64[D]")
        # convert user string to numpy datetime64[D]
        try:
            target = np.datetime64(pd.to_datetime(date_str).date(), "D")
        except Exception:
            # fallback: if unparseable, use first or last
            if date_str.lower() == "first":
                return 0
            if date_str.lower() == "last":
                return int(len(all_times) - 1)
            # try a direct cast anyway
            target = np.datetime64(date_str, "D")
        # find nearest index
        deltas = np.abs(all_times - target)
        idx = int(deltas.argmin())
        # clamp
        idx = max(0, min(idx, len(all_times) - 1))
        return idx

    idx0 = _to_idx(date0_str)
    idx1 = _to_idx(date1_str)
    if idx1 <= idx0:
        idx1 = min(idx0 + 1, len(ds[time_dim]) - 1)
    step += 1
    if progress_callback:
        progress_callback(step / total_steps)
    ...
    time_summary: pd.Series = compute_time_summary(ds[time_dim])
    band_summary: pd.DataFrame = compute_band_summary(ds)
    step += 1
    if progress_callback:
        progress_callback(step / total_steps)
        

    # 6) Prepare a local output folder:
    out_base = Path("inspect_outputs") / f"{geohash}_{date0_str.replace('-','')}_{date1_str.replace('-','')}"
    ...
    # 7) Pull out true‐color at idx0 & idx1, then save as PNGs

    # 7a) Determine which variables hold R/G/B
    var_map = {v.lower(): v for v in ds.data_vars}
    for channel in ("r","g","b"):
        if channel not in var_map:
            raise ValueError(f"Cannot find RGB band '{channel}' in dataset; available vars: {list(ds.data_vars)}")

    rgb_bands = [var_map["r"], var_map["g"], var_map["b"]]

    def _make_rgb_png(at_index: int, fname: Path):
        """
        Extract (H,W,3) array at time=at_index, stretch to [0,1], and save PNG to fname.
        """
        arr_stack = np.stack([
            ds[band].isel({time_dim: at_index}).values
            for band in rgb_bands
        ], axis=0)  # shape = (3, H, W)

        # Simple percentile stretch (1st–99th) for each channel:
        rgb_img = np.moveaxis(arr_stack, 0, -1).astype(float)  # shape = (H, W, 3)
        for ch in range(3):
            onechan = rgb_img[..., ch]
            lo, hi = np.percentile(onechan, (1, 99))
            if hi - lo < 1e-6:
                stretch = np.clip((onechan - lo), 0, 1)
            else:
                stretch = np.clip((onechan - lo) / (hi - lo), 0, 1)
            rgb_img[..., ch] = stretch
    ...
    rgb_t0_png = out_base / "rgb_t0.png"
    rgb_t1_png = out_base / "rgb_t1.png"
    _make_rgb_png(idx0, rgb_t0_png)
    _make_rgb_png(idx1, rgb_t1_png)
    step += 1
    if progress_callback:
        progress_callback(step / total_steps)

    # 8) Compute an NDVI difference = NDVI(idx1) – NDVI(idx0), save as PNG:
    #    (Assumes “ndvi” is present; if not, skip gracefully.)
    ndvi_png = None
    if "ndvi" in ds.data_vars:
        arr_ndvi0 = ds["ndvi"].isel({time_dim: idx0}).values
        arr_ndvi1 = ds["ndvi"].isel({time_dim: idx1}).values
        diff_ndvi = arr_ndvi1 - arr_ndvi0  # shape = (H, W)

        # Clip‐to‐percentile for visualization
        vmin = np.nanpercentile(diff_ndvi, 2)
        vmax = np.nanpercentile(diff_ndvi, 98)
        ndvi_png = out_base / "ndvi_diff.png"
        plt.figure(figsize=(6, 6))
        im = plt.imshow(diff_ndvi, cmap="RdYlGn", vmin=vmin, vmax=vmax, origin="upper")
        plt.axis("off")
        plt.colorbar(im, fraction=0.04, pad=0.02)
        plt.savefig(ndvi_png, bbox_inches="tight", pad_inches=0)
        plt.close()
    step += 1
    if progress_callback:
        progress_callback(step / total_steps)
    ...
    # 9) Gather dims and data_vars into simple lists
    data_vars = list(ds.data_vars)
    step += 1
    if progress_callback:
        progress_callback(step / total_steps)
        
    # 10) Return everything in a dict for Streamlit to use:
    out_dict = {
        "dims":                dims,                   # e.g. {'time': 10, 'y': 512, 'x': 512}
        "data_vars":           data_vars,              # e.g. ['r','g','b','ndvi','elevation',…]
        "time_summary":        time_summary,           # a pandas Series (min,max,count,…)
        "band_summary":        band_summary,           # a pandas DataFrame (min,max,mean,std,median for every var)
        "rgb_t0_png":          str(rgb_t0_png.resolve()),
        "rgb_t1_png":          str(rgb_t1_png.resolve()),
        "ndvi_diff_png":       str(ndvi_png.resolve()) if ndvi_png is not None else None
    }
    return out_dict
